...
Мы определили сервис под названием `web`, для которого надо использовать образ flask-datamove и пробросить порты заданным образом.
...
Мы пока получили такой же результат, как и запуская контейнер командой `docker run`, но выгода уже очевидна - мы записали опции для этой команды в файл и терь нам не надо их заново печатать каждый раз.

Мы дали опцию `-p $USER` (название проекта), чтобы не было конфликта названий контейнера. По умолчанию в качестве проекта используется имя текущей папки, т.е. flask в нашем случае. Без этой опции у все, у кого проек в папке flask, использовался бы одно название котенй
...
Мы определили сервис по названием `web`, для которого надо собрать образ используя Dockerfile и контеск из текущей директории.
...
Мы их можем индивиуально подергать на своих портах.
...
Создайте новый файл docker-compose-nginx.yml (или скопируйте -rest.yml) и отредактируйте (заменяя ник, порт на ваши):
...
Сдесь мы делаем маршрутизацию по имени хоста. Все запросы к www.example.com должны направляться в контейнер web, а все запросы к rest.example.com - в контейнер rest. Запросы на другие хосты не обрабатывабтся - возвращается ошибка 422. Мы можем обращаться к нашим фласк-серверам в контейнерах по именам контейнеров, так как внутри виртуальной сети докера контейнеры - это хосты, а их имена соответствуют названиям сервисов. 
...
К слову сказать, так же можно следать маршрутизацию по url (location /) или даже используя заголовки запросов:
